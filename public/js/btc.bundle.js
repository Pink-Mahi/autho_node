(() => {
  ...
  function weierstrassN(params, extraOpts = {}) {
    ...
    class Point3 {
      ...
      static fromHex(hex2) {
        return Point3.fromBytes(ensureBytes("pointHex", hex2));
      }
      get x() {
  ...
  var isBytes4 = utils2.isBytes;
  var concatBytes3 = utils2.concatBytes;
  var equalBytes2 = utils2.equalBytes;
...
zoo`.split("\n");
  ...
  function getBip84Path(network, account = 0, index = 0) {
  ...
  function derivePrivKeyBytesFromMnemonic(mnemonic, network, account = 0, index = 0) {
  ...
  function getP2WPKHAddressFromMnemonic(mnemonic, network, account = 0, index = 0) {
    const priv = derivePrivKeyBytesFromMnemonic(mnemonic, network, account, index);
    const pub = secp256k1.getPublicKey(priv, true);
    const pay = p2wpkh(pub, getNetwork(network));
    if (!pay.address)
      throw new Error("Failed to derive address");
    return pay.address;
  }
  function getPublicKeyFromMnemonic(mnemonic, network, account = 0, index = 0) {
    const priv = derivePrivKeyBytesFromMnemonic(mnemonic, network, account, index);
    const pub = secp256k1.getPublicKey(priv, true);
  ...
  function normalizeUtxosForTx(mnemonic, utxos, network, account = 0, index = 0) {
    const priv = derivePrivKeyBytesFromMnemonic(mnemonic, network, account, index);
    const pub = secp256k1.getPublicKey(priv, true);
    const spend = p2wpkh(pub, getNetwork(network));
    return utxos.map((u) => ({
      ...spend,
  ...
  function buildAndSignP2WPKH(params) {
    ...
    if (!outputsInput) {
      ...
      if (!Number.isFinite(params.amountSats) || params.amountSats <= 0)
        throw new Error("Invalid amountSats");
    }
    const hasScriptOutputs = !!(outputsInput && outputsInput.some((o) => o && typeof o.scriptHex === "string"));
    ...
    const outputs = outputsInput ? outputsInput.map((o) => {
      const amountSats = Number(o?.amountSats || 0);
      if (!Number.isFinite(amountSats) || amountSats < 0)
        throw new Error("Invalid output amountSats");
    ...
    const selected = selectUTXO(utxo, outputs, "default", {
      changeAddress,
    ...
    if (needsFineTuning) {
      ...
      const changeScript = OutScript.encode(Address(getNetwork(network)).decode(changeAddress));
      let changeOutIdx = -1;
      ...
      if (currentFeeSats !== desiredFeeSats) {
        ...
        const currentChange = BigInt(changeOut?.amount || 0n);
        if (currentFeeSats > desiredFeeSats) {
          const delta = currentFeeSats - desiredFeeSats;
          const newChange = currentChange + delta;
          selected.tx.updateOutput(changeOutIdx, { ...changeOut, amount: newChange }, true);
        } else {
          const delta = desiredFeeSats - currentFeeSats;
          const newChange = currentChange - delta;
          if (newChange <= 0n) {
