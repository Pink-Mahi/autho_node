...
export class RegistryAPI {
  ...
  async getItem(req: Request, res: Response): Promise<void> {
    try {
      ...
      let mintedByOfficialManufacturer = Boolean((itemRecord as any)?.mintedByOfficialManufacturer);
      
      // Enhanced detection: check if manufacturerId matches an approved manufacturer account
      if (this.canonicalStateBuilder) {
        try {
          const state = canonicalState || await this.canonicalStateBuilder.buildState();
          
          // First check issuerAccountId, then manufacturerId
          const issuerId = String((itemRecord as any)?.issuerAccountId || (itemRecord as any)?.manufacturerId || '').trim();
          const manufacturerId = String((itemRecord as any)?.manufacturerId || '').trim();
          
          // Check if issuer is a manufacturer or authenticator
          if (issuerId) {
            const issuerAccount: any = state.accounts.get(issuerId);
          ...
          // Also check manufacturerId separately (for items where manufacturerId != issuerAccountId)
          if (!mintedByOfficialManufacturer && manufacturerId) {
            const mfgAccount: any = state.accounts.get(manufacturerId);
            if (mfgAccount && String(mfgAccount.role || '').trim() === 'manufacturer') {
              // The manufacturerId is an approved manufacturer, so this item is manufacturer-verified
              mintedByOfficialManufacturer = true;
          ...
          console.error('[Registry API] Error detecting issuer role:', e);
          if (!issuerRole) issuerRole = 'user';
        }
      }
